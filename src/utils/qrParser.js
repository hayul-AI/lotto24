...
export const parseLottoQr = (decodedText) => {
  ...
  // 로또 QR은 반드시 알파벳 구분자(m, q, s 등)를 포함함
  if (!v || !/[a-z]/i.test(v)) {
  ...
  if (!Number.isFinite(drawNo) || drawNo < 1) {
...
/**
 * 연금복권 720+ 파싱
 * 예: https://m.dhlottery.co.kr/qr.do?method=pension720&v=11000123456
 */
export const parsePensionQr = (decodedText) => {
  ...
  try {
    ...
    const match = rawQr.match(/v=([0-9]{11})/);
    v = match ? match[1] : "";
    if (!v) {
